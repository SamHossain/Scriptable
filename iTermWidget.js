...
const NAME = 'Sam';
// an api key can be obtained from https://openweathermap.org
...
function createWidget(data) {
  ...
  //modyfy these to make the 'login' line shorter
  timeFormatter.useShortTimeStyle();
  timeFormatter.useMediumDateStyle();
  ...
  // Line 6a - Period
  //can add in the same format with ${data.period}
  //Line 6b - Stock
  const stockLine = stack.addText(`📈 | SPY: ${data.stock.price} (${data.stock.changepercent}%)`);
  stockLine.textColor = (data.stock.changepercent <= 0) ?new Color(COLORS.period) : new Color(COLORS.pos);   
  stockLine.font = new Font(FONT_NAME, FONT_SIZE);
...
// stock data  
  const stock = await fetchStockData(STOCK_TICKER);
...
function getCalendarEventTitle(calendarEvent, isWorkEvent) {
  ...
  const timeFormatter = new DateFormatter();
  timeFormatter.locale = 'en';
  //useNoDateStyle() to have no date shown
...
/*
*Stock query helper functions
*/

async function getStockData(symbol) { 
  
    let stkdata = await queryStockData(symbol.trim());
